...
export default function DispatchPage() {
  ...
  const isPickup = (order: { fulfillmentMode?: string }) => order.fulfillmentMode === "PICKUP";
  const dispatchable = assignments.filter((order) => !isPickup(order));
  const metrics = {
    pickup: assignments.filter(isPickup).length,
    pending: dispatchable.filter((order) => order.status === "PENDING").length,
    assigned: dispatchable.filter((order) => order.status === "ASSIGNED").length,
    scheduled: dispatchable.filter((order) => order.status === "SCHEDULED").length,
    unassigned: dispatchable.filter((order) => !order.driverId).length,
  };
  const visible = useMemo(() => assignments.filter((order) => view === "PICKUP" ? isPickup(order) : !isPickup(order) && (view === "ALL" || (view === "UNASSIGNED" && !order.driverId) || (view === "ASSIGNED" && order.driverId) || (view === "SCHEDULED" && order.status === "SCHEDULED"))), [assignments, view]);
