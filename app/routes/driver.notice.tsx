...
export default function DriverNotice() { const { announcements } = useLoaderData<typeof loader>(); return <main style={{ maxWidth: 760, margin: "0 auto", padding: "48px 20px" }}><p><Link to="/driver">← Driver portal</Link></p><h1>Driver notices</h1>{announcements.length === 0 ? <p>No current notices.</p> : announcements.map((a) => <article key={a.id}><h2>{a.title}</h2><p>{a.body}</p></article>)}</main>; }
