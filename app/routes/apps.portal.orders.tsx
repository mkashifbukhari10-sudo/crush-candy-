...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const withChats = await Promise.all(orders.map(async (order) => { try { const chat = await ensureOrderConversation(order.id); return { ...order, conversationId: chat.id, pickupEligible: isPickupEligible(order.lineItems) }; } catch { return { ...order, conversationId: null, pickupEligible: isPickupEligible(order.lineItems) }; } }));
  return { orders: withChats };
}
export default function CustomerOrders() { const { orders } = useLoaderData<typeof loader>(); return <><h1>Delivery status</h1>{orders.length === 0 ? <p>No operational orders yet.</p> : <ul>{orders.map((order) => <li key={order.id}><strong>{order.shopifyOrderNumber}</strong> — {order.status}{order.scheduledFor ? ` · scheduled ${new Date(order.scheduledFor).toLocaleString("en-AU", { timeZone: "Australia/Perth" })}` : " · processing"}{order.conversationId ? <> · <Link to={`/apps/portal/chat/${order.conversationId}`}>Open delivery chat</Link></> : null}{order.pickupEligible ? <> · <Link to={`/apps/portal/pickup/${order.id}`}>Arrange pickup</Link></> : null}</li>)}</ul>}<p><Link to="/apps/portal">Back to portal</Link></p></>; }
