...
export async function listAllAnnouncements() { return db.announcement.findMany({ orderBy: [{ audience: "asc" }, { sortOrder: "asc" }, { createdAt: "desc" }], take: 200 }); }
...
export async function listCustomerTickets(customerId: string) { return db.supportTicket.findMany({ where: { shopifyCustomerId: customerId }, orderBy: { createdAt: "desc" }, take: 100, include: { messages: { orderBy: [{ createdAt: "asc" }, { id: "asc" }], take: 200 } } }); }
export async function listSupportTickets() { return db.supportTicket.findMany({ orderBy: { updatedAt: "desc" }, take: 200, include: { messages: { orderBy: [{ createdAt: "asc" }, { id: "asc" }], take: 200 } } }); }
