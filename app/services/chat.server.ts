...
export async function listCustomerConversations(customerId: string) { return db.conversation.findMany({ where: { shopifyCustomerId: customerId, status: "OPEN" }, include: { assignment: { select: { shopifyOrderNumber: true } }, messages: { orderBy: { createdAt: "desc" }, take: 1 } }, orderBy: { updatedAt: "desc" } }); }
export async function listDriverConversations(driverId: string) { return db.conversation.findMany({ where: { status: "OPEN", assignment: { driverId, status: { in: [...OPEN_STATUSES] } } }, include: { assignment: { select: { shopifyOrderNumber: true } }, messages: { orderBy: { createdAt: "desc" }, take: 1 } }, orderBy: { updatedAt: "desc" } }); }
